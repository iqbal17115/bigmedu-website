        ...
        <div class="container">
            ...
            <div class="sixteen columns" id="left-content">
                ...
                <div id="with-pic" style="display: block;">
                    <h4 style="color: #434489;">BIGM</h4>
                    <table class="bordered" id="categorized_list">
                        <tbody>
                            @foreach($allBIGM as $single)
                            <tr>
                                <td style="width: 3%;" class="idHide">{{ $loop->iteration }}</td>
                                <td style="width: 12%;"><img src="{{ asset('public/upload/members/'.$single['member']->image) }}" style="height: 100px; width: 100%;"></td>
                                <td style="width: 40%;">
                                    <table>
                                        <tbody>
                                            <tr>
                    ...
                    @foreach($projects as $project)
                        @php
                            $allProject = \App\Model\MemberToEmployee::with('member')->where('dept_or_project',2)->where('project_id',$project->id)->orderBy('sort_order','ASC')->get();
                        @endphp

                        @if(count($allProject))
                            <h4 style="margin-top: 10px;color: #434489;">{{ $project->project_name }} Project</h4>
                        @endif

                        <table class="bordered" id="categorized_list">
                            <tbody>
                                @foreach($allProject as $single)
                                <tr>
                                    <td style="width: 3%;" class="idHide">{{ $loop->iteration }}</td>
                                    <td style="width: 12%;"><img src="{{ asset('public/upload/members/'.$single['member']->image) }}" style="height: 100px; width: 100%;"></td>
                                    <td style="width: 40%;">
            ...
            <br>
            @php
            $lastMemberToEmployee = \App\Model\MemberToEmployee::with('member')->orderBy('id','DESC')->first();
